...
class MaskDecoderAuto(nn.Module):
    def __init__(
        ...
        *,
        ...
        """
          Predicts masks given an image embedding, using a
    ...
    def predict_masks(
        ...
        # Build only the output tokens (iou + mask tokens), without prompts
        ...
        upscaled_embedding = self.output_upscaling(src) #(B,C,256,256)

        # Hypernetworks to project mask token → filter
        hyper_in_list = []
        for i in range(self.num_mask_tokens):
