...
class LMHSA(nn.Module):
    ...
    def __init__(self, input_size, channels, d_k, d_v, stride, heads, dropout):
        ...
        # FIX: B viene creato dinamicamente nel forward basandosi sulle dimensioni reali
        # Salviamo solo input_size per riferimento
        self.input_size = input_size
        self.B = None  # Verrà inizializzato al primo forward
    ...
    def forward(self, x):
        ...
        # FIX: Inizializza B con le dimensioni corrette al primo forward
        if self.B is None or self.B.shape[2] != h * w or self.B.shape[3] != k_h * k_w:
            self.B = nn.Parameter(
                torch.zeros(1, self.heads, h * w, k_h * k_w, device=x.device, dtype=x.dtype),
                requires_grad=True
            )
